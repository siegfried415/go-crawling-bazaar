...
func CreateDomain(meta ResourceMeta) (txHash hash.Hash, dsn string, err error) {
	if atomic.LoadUint32(&driverInitialized) == 0 {
	...
	if err = requestPB(route.MCCNextAccountNonce, nonceReq, nonceResp); err != nil {
	...
	if err = requestPB(route.MCCAddTx, req, resp); err != nil {
...
func WaitDomainCreation(ctx context.Context, dsn string) (err error) {
	...
	// wait for creation
	err = WaitPBDomainCreation(ctx, proto.DatabaseID(dsnCfg.DatabaseID), db, 3*time.Second)
	return
}
...
func GetTokenBalance(tt types.TokenType) (balance uint64, err error) {
	...
	if err = requestPB(route.MCCQueryAccountTokenBalance, req, resp); err == nil {
		if !resp.OK {
...
func UpdatePermission(targetUser proto.AccountAddress,
	...
	addTxReq.Tx = up
	err = requestPB(route.MCCAddTx, addTxReq, addTxResp)
	if err != nil {
...
func WaitTxConfirmation(
	...
	defer fmt.Printf("\n")
	for {
		if err = requestPB(method, req, resp); err != nil {
...
func getNonce(addr proto.AccountAddress) (nonce interfaces.AccountNonce, err error) {
	...
	nonceResp := new(types.NextAccountNonceResp)
	nonceReq.Addr = addr
	err = requestPB(route.MCCNextAccountNonce, nonceReq, nonceResp)
	if err != nil {
