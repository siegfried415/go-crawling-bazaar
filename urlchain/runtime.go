...
func (r *runtime) stop(domainID proto.DomainID) {
	r.stopService(domainID)
	r.cancel()
	r.wg.Wait()
}
...
func (r *runtime) startService(chain *Chain) {
	r.muxService.register(chain.domainID, &ChainRPCService{chain: chain})
}

func (r *runtime) stopService(domainID proto.DomainID) {
	r.muxService.unregister(domainID)
}
