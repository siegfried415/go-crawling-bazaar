...
func NewService(host host.Host, database string, server *rpc.Server) (s *Service, err error) {
	var (
...
func (s *Service) pull(count int32) (err error) {
	...
	//if err = rpc.NewCaller().CallNode(s.upstream, route.DBSObserverFetchBlock.String(), req, resp); err != nil {
        s, err := s.host.NewStream(ctx, s.upstream, route.DBSObserverFetchBlock.String() )
		return
	}
        defer s.Close()
	...
	if err != nil {
		err = errors.Wrap(err, "send DBS.ObserverFetchBlock failed")
		return 
	}

	resp, err := ioutil.ReadAll(s)
	if err!= nil {
		err = errors.Wrap(err, "get DBS.ObserverFetchBlock result failed")
		return 
	} 
