...
func NewService(host host.Host ) (service *Service, err error) {
...
func (s *Service) minerRequest(dbID proto.DatabaseID, method string, request interface{}, response interface{}) (err error) {
	...
	//return s.caller.CallNode(instance.Peers.Leader, method, request, response)
        s, err := s.host.NewStream(ctx, instance.Peers.Leader, method )
	if err != nil {
		err = errors.Wrap(err, "call miner request failed")
		return
	}
        defer s.Close()

	_, err := net.MsgToStream(ctx, s, &request )
	if err != nil {
		err = errors.Wrap(err, "send miner request failed")
		return 
	}

	response, err := ioutil.ReadAll(s)
	if err!= nil {
		err = errors.Wrap(err, "get miner result failed")
		return 
	} 
...
func (s *Service) getUpstream(dbID proto.DatabaseID) (instance *types.ServiceInstance, err error) {
	...
	*/
	...
	if err != nil {
		err = errors.Wrap(err, "send MCC.QuerySQLChainProfile failed")
		return 
	}

	resp, err := ioutil.ReadAll(s)
	if err!= nil {
		err = errors.Wrap(err, "get MCC.QuerySQLChainProfile result failed")
		return 
	} 
