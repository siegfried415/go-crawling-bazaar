...
func (s *metaState) loadOrStoreSQLChainObject(
	k proto.DomainID, v *types.SQLChainProfile) (o *types.SQLChainProfile, loaded bool,
) {
	if o, loaded = s.dirty.domains[k]; loaded && o != nil {
		return
	}
	if o, loaded = s.readonly.domains[k]; loaded {
		return
	}
	s.dirty.domains[k] = v
	return
...
func (s *metaState) deleteSQLChainObject(k proto.DomainID) {
	// Use a nil pointer to mark a deletion, which will be later used by commit procedure.
	s.dirty.domains[k] = nil
}
...
func (s *metaState) commit() {
	...
	for k, v := range s.dirty.domains {
		if v != nil {
			// New/update object
			s.readonly.domains[k] = v
		} else {
			// Delete object
			delete(s.readonly.domains, k)
		}
	}
...
func (s *metaState) createSQLChain(addr proto.AccountAddress, id proto.DomainID) error {
	if _, ok := s.dirty.accounts[addr]; !ok {
		if _, ok := s.readonly.accounts[addr]; !ok {
			return ErrAccountNotFound
		}
	}
	if _, ok := s.dirty.domains[id]; ok {
		return ErrDatabaseExists
	} else if _, ok := s.readonly.domains[id]; ok {
		return ErrDatabaseExists
	}
	s.dirty.domains[id] = &types.SQLChainProfile{
		ID:     id,
		Owner:  addr,
...
func (s *metaState) addSQLChainUser(
	k proto.DomainID, addr proto.AccountAddress, perm *types.UserPermission) (_ error,
) {
	var (
		src, dst *types.SQLChainProfile
		ok       bool
	)
	if dst, ok = s.dirty.domains[k]; !ok {
		if src, ok = s.readonly.domains[k]; !ok {
			return ErrDatabaseNotFound
		}
		dst = deepcopy.Copy(src).(*types.SQLChainProfile)
		s.dirty.domains[k] = dst
	}
...
func (s *metaState) deleteSQLChainUser(k proto.DomainID, addr proto.AccountAddress) error {
	var (
		src, dst *types.SQLChainProfile
		ok       bool
	)
...
//wyong, 20200728 
func (s *metaState) matchProvidersWithUser(tx *types.CreateDomain) (err error) {
        //wyong, 20200612
        log.WithFields(log.Fields{
                "transaction":       tx,
	...
	domainAddr, err := dID.AccountAddress()
...
func isProviderReqMatch(po *types.ProviderProfile, req *types.CreateDomain) (match bool, err error) {
...
func (s *metaState) updatePermission(tx *types.UpdatePermission) (err error) {
	...
	so, loaded := s.loadSQLChainObject(tx.TargetSQLChain.DomainID())
	if !loaded {
		log.WithFields(log.Fields{
			"domainID": tx.TargetSQLChain.DomainID(),
	...
	s.dirty.domains[tx.TargetSQLChain.DomainID()] = so
	return
}
...
func (s *metaState) updateBilling(tx *types.UpdateBilling) (err error) {
	newProfile, loaded := s.loadSQLChainObject(tx.Receiver.DomainID())
	if !loaded {
	...
	s.dirty.domains[tx.Receiver.DomainID()] = newProfile
	return
}
...
func (s *metaState) transferSQLChainTokenBalance(transfer *types.Transfer) (err error) {
	...
	sqlchain, ok = s.loadSQLChainObject(transfer.Receiver.DomainID())
	if !ok {
		err = ErrDatabaseNotFound
		log.WithFields(log.Fields{
			"dbid":   transfer.Receiver.DomainID(),
			"sender": transfer.Sender,
	...
	for _, user := range sqlchain.Users {
		if user.Address == transfer.Sender {
			...
			s.dirty.domains[transfer.Receiver.DomainID()] = sqlchain
...
func (s *metaState) applyTransaction(tx pi.Transaction, height uint32) (err error) {
        ...
        log.WithFields(log.Fields{
                "transaction":       tx,
                }).Debug("blockproducer/metastate, applyTransaction() called")
...
func (s *metaState) generateGenesisBlock(domainID proto.DomainID, tx *types.CreateDomain) (genesisBlock *types.Block, err error) {
