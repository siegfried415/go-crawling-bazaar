...
func (rpc *FronteraRPCService) ObserverFetchBlock(req *ObserverFetchBlockReq, resp *ObserverFetchBlockResp) (err error) {
	subscriberID := req.GetNodeID().ToNodeID()
	resp.Block, resp.Count, err = rpc.frontera.observerFetchBlock(req.DatabaseID, subscriberID, req.Count)
	return
}

func (f *Frontera) observerFetchBlock(dbID proto.DatabaseID, nodeID proto.NodeID, count int32) (
	...
	// check permission
	err = f.checkPermission(addr, dbID, types.ReadQuery, nil)
	if err != nil {
		log.WithFields(log.Fields{
			"databaseID": dbID,
			"addr":       addr,
