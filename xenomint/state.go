...
func readSingle(
	...
	log.WithFields(log.Fields{
                                "pattern":       pattern,
                                "args":        args,
                        }).Debug("xenomint/state, readSingle() called")
...
func (s *State) writeSingle(
	...
	log.WithFields(log.Fields{
                                "pattern":       pattern,
                                "args":        args,
                        }).Debug("xenomint/state, writeSingle() called")
	...
	if err = func() (err error) {
		...
		if isLeader {
			...
			log.WithFields(log.Fields{
						"lastSeq":       lastSeq,
						"query":        query,
					}).Debug("xenomint/state.write() called, enqueue query to s.pool ")
...
func (s *State) Stat(id proto.DomainID) {
	var (
		p = func() *pool {
			s.RLock()
			defer s.RUnlock()
