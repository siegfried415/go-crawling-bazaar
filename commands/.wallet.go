...
func showDatabaseDeposit(host host.Host, dsn string) {
	...
	req.DomainID = proto.DomainID(dsnCfg.DomainID)

	//wyong, 20201008 
	if err = net.RequestBP(host, route.MCCQuerySQLChainProfile.String(), req, resp); err != nil {
	...
	if addr, err = address.PubKeyHash(pubKey); err != nil {
		ConsoleLog.WithError(err).Error("query database chain profile failed")
		SetExitStatus(1)
		return
	}
...
func showAllDatabaseDeposit(host host.Host ) {
	var (
	...
	if req.Addr, err = address.PubKeyHash(pubKey); err != nil {
		ConsoleLog.WithError(err).Error("query database chain profile failed")
		SetExitStatus(1)
		return
	}
	...
	//todo, wyong, 20201008 
	if err = net.RequestBP(host, route.MCCQueryAccountSQLChainProfiles.String(), req, resp); err != nil {
...
func runWallet(cmd *Command, args []string) {
	...
	if databaseID != "" {
		showDatabaseDeposit(host, databaseID)
	} else if tokenName == "" {
		showAllTokenBalance()

		//wyong, 20201008 
		showAllDatabaseDeposit(host)
	} else {
